...
%delta = mMax - mMin;
%image_normalized = (result - mMin) ./ delta;
%range2 = max_norm - min_norm;
%result = (image_normalized*range2) + min_norm;
white = 65535;
result = double(input);
result(result<blackLevel)=0;
result(result>white)=white;
result = (result-blackLevel)./(white-blackLevel);
